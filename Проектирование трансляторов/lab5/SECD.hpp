...
/*
 * LD a b - Поиск переменной в окружении, где a - номер подсписка, b - позиция переменной
 * LDC - Константа
 * OP - Операция
 * SEL - Выбор
 * JOIN - Подставка учаска кода
 * LDF - Вызов лямбды
 * AP - Формирование вычислительной обстановки
 * RTN - Возврат к состоянию в регистре d
 * STOP
*/
...
using Answer_Value = std::variant<int, std::string, std::shared_ptr<List>, std::shared_ptr<Lambda>, bool>;
...
class Answer
{
    ...
    bool get_bool()
    {
        if (!this->is_bool()) throw std::runtime_error("not a bool");
...
using Arg_Type = std::variant<int, std::string, std::vector<Arg_Type>>;
//using pArg_Type = std::shared_ptr<Arg_Type>;
...
struct Command
{
    ...
    Command(int com_tok, std::string com_name = "")
    {
        this->com_tok = com_tok;
        this->com_name = com_name;
        this->args = {};
    }

    // template <typename... Args>
    // Command(int com, std::string com_name = "", Args... args)
    // {
    //     this->com_tok = com;  
    //     this->com_name = com_name;
    //     this->args = {args...};
    ...
    void print()
    {
        std::cout << "Command: " << com_name << "| Token: " << com_tok << "" << "\n";
        std::cout << "Args: ";
...
using Stack_Type = std::stack<Answer>;
using Enviroment_Type = void*;
using Control_list_Type = std::vector<Command>;

using pStack_Type = std::shared_ptr<Stack_Type>;
using pEnviroment_Type = std::shared_ptr<Enviroment_Type>;
using pControl_list_Type = std::shared_ptr<Control_list_Type>;


using Dump_Type = std::stack<SECD>;
using pDump_Type = std::shared_ptr<Dump_Type>;
