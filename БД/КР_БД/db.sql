...
CREATE TABLE IF NOT EXISTS `mydb`.`Supply` (
  ...
  CONSTRAINT `fk_Supply_Worker1`
    FOREIGN KEY (`Worker_id`)
    REFERENCES `mydb`.`Worker` (`id`),
  CONSTRAINT `fk_Supply_Company1`
    FOREIGN KEY (`Company_INN`)
    REFERENCES `mydb`.`Company` (`INN`));
...
DELIMITER //
CREATE PROCEDURE get_product_price(IN in_product_id MEDIUMINT, IN in_supply_id SMALLINT, OUT product_price DECIMAL(7,2), OUT is_eatable BOOL)
BEGIN
    select retail_price into product_price from invoice 
	join product on (product.id = invoice.product_id)
	join supply on (supply.id = invoice.Supply_id)
	where supply_id = in_supply_id and product_id = in_product_id;
    
    select shortage_life >= (datediff(now(), supply.`date`)) into is_eatable from invoice 
	join product on (product.id = invoice.product_id)
	join supply on (supply.id = invoice.Supply_id)
	where supply_id = in_supply_id and product_id = in_product_id;
END //
...
insert into company(INN, name, city, is_manufacturer, is_supplier, phone) values("1231231231", "ИП Харламов М.В.", "Орел", false, true, "88005553535");
insert into company(INN, name, city, is_manufacturer, is_supplier, phone) values("3456541239", "Логистический партнёр", "Орел", false, true, "88005553535");
insert into company(INN, name, city, is_manufacturer, is_supplier, phone) values("7654265879", "Опт-Торг", "Орел", false, true, "88005553535");

insert into company(INN, name, city, is_manufacturer, is_supplier, phone) values("3452346788", "Сельскохозяйственная Марёвская Корпорация", "Марево", true, false, "88005553535");
insert into company(INN, name, city, is_manufacturer, is_supplier, phone) values("1123323244", "ООО МЯСОПРОДУКТ", "Москва", true, false, "88005553535");
insert into company(INN, name, city, is_manufacturer, is_supplier, phone) values("3456765423", "ООО Правильные продукты", "Абакан", true, false, "88005553535");

insert into company(INN, name, city, is_manufacturer, is_supplier, phone) values("3234566522", "ООО Хэппи-Фиш", "Астрахань", true, true, "88005553535");
...
insert into product(name, shortage_life, Company_INN, Wrap_id, extra, retail_price) values(
"Сметана Простоквашино",                                             #name
35,                                                                  #shortage life
(select INN from company where name = "ООО Правильные продукты"),    #company
...
insert into supply(date, Company_INN, worker_id) values(
...
select retail_price, datediff(date(now()), `date`) as days_count, shortage_life from invoice 
join product on (product.id = invoice.product_id)
join supply on (supply.id = invoice.Supply_id)
